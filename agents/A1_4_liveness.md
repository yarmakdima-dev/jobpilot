...
- `roles/*.json` — read `source_url` and `role_id` for roles in state `sourced` with `liveness: null`
...
```json
{
  "liveness": "active | dead | unknown",
  "liveness_checked_at": "ISO-8601",
  "liveness_evidence": "string — one sentence: what the snapshot showed"
}
```
...
1. For each role record with `liveness: null` and `pipeline_state: "sourced"`:
2. Navigate to `source_url` using Playwright. Capture a DOM snapshot.
...
5. Signal orchestrator: `active` → advance to F1; `dead` → close role, log reason; `unknown` → hold for human review, surface in daily report.
...
- Response time per URL ≤ 15 seconds. If Playwright times out, record `unknown`.
...
- Do not write any field other than `liveness`, `liveness_checked_at`, and `liveness_evidence`.
