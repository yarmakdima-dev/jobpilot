...
All Gemini API calls are mocked via agents.a0.gemini_client.call_research.
The file-system store is exercised through a tmp-path root so every test
is isolated and repeatable.
"""
...
_FIXTURES = Path(__file__).parent / "fixtures" / "a0"
...
def _clean_profile() -> dict:
    ...
    with (_FIXTURES / "gemini_response_clean.json").open(encoding="utf-8") as fh:
        return json.load(fh)
...
def _ru_leader_profile() -> dict:
    ...
    profile["snapshot"]["domain"] = "acme.com"
...
def _make_gemini_response(profile: dict, sources: list[str] | None = None):
    """Construct a mock GeminiResponse."""
    ...
    return GeminiResponse(
        content=profile,
        sources=sources,
        usage={"prompt_tokens": 800, "output_tokens": 400, "total_tokens": 1200},
    )
...
def jobpilot_root(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    """Isolated store root with all required files."""
...
@pytest.fixture()
def mock_gemini_clean(jobpilot_root):
    """Patch call_research to return the clean fixture profile.

    Used by the majority of tests that just need a valid A0 response.
    """
    with patch(
        "agents.a0.research.call_research",
        return_value=_make_gemini_response(_clean_profile()),
    ) as m:
        yield m
...
def test_new_company_creates_profile(mock_gemini_clean, jobpilot_root: Path) -> None:
...
def test_new_company_logs_decision(mock_gemini_clean, jobpilot_root: Path) -> None:
...
def test_existing_fresh_company_noop(mock_gemini_clean, jobpilot_root: Path) -> None:
    ...
    # Second run — must be no-op (call_research not called again)
    run_a0(role)
    mtime_2 = profile_path.stat().st_mtime

    assert mtime_2 == mtime_1, "Profile must not be re-written when fresh"
    assert mock_gemini_clean.call_count == 1, "Gemini must only be called once"


def test_fresh_company_no_additional_decisions(mock_gemini_clean, jobpilot_root: Path) -> None:
...
def test_stale_company_reruns_research(mock_gemini_clean, jobpilot_root: Path) -> None:
    ...
    assert mock_gemini_clean.call_count == 2, "Gemini should be called again for stale profile"

    refreshed = json.loads(profile_path.read_text(encoding="utf-8"))
    assert not refreshed.get("synthesis_stale")


def test_stale_company_logs_second_decision(mock_gemini_clean, jobpilot_root: Path) -> None:
...
# ── Schema validation: invalid / malformed responses ─────────────────────────


def test_invalid_response_raises_schema_error(jobpilot_root: Path) -> None:
    """A0SchemaError from Gemini (bad JSON) propagates out of run_a0."""
    from agents.a0.gemini_client import A0SchemaError

    with (
        patch("agents.a0.research.call_research", side_effect=A0SchemaError("bad json")),
        pytest.raises(A0SchemaError),
    ):
        role = _make_a0_role()
        _write_role_a1(role)
        run_a0(role)
...
def test_missing_required_section_raises(jobpilot_root: Path) -> None:
    ...
    incomplete = {
        "instance_meta": {
            ...
            "target_role": "COO",
            "role_url": "https://acme.com/jobs",
            "research_depth": "light",
            "researcher": "A0",
            "confidence_notes": "incomplete",
        },
        "snapshot": {"legal_name": "Acme", "domain": "acme.com", "hq": "NYC",
                     "primary_market": "USA", "sector": "SaaS", "sub_sector": "Tooling",
                     "secondary_domains": []},
        # deliberately missing: name_confusion_check, source_index, gate_needs_judgment_call
    }
...
def test_judgment_call_set_for_ru_silent_leader(jobpilot_root: Path) -> None:
    """RU-origin CEO with silent war_position triggers judgment call on role."""
    mock_resp = _make_gemini_response(_ru_leader_profile())
    with patch("agents.a0.research.call_research", return_value=mock_resp):
        role = _make_a0_role()
        _write_role_a1(role)
        result = run_a0(role)

    assert "gate_needs_judgment_call" in result
...
def test_judgment_call_set_for_ru_parallel_business(jobpilot_root: Path) -> None:
    """Leader with active RU parallel business (carve_out_escalation) triggers judgment call."""
    mock_resp = _make_gemini_response(_ru_parallel_business_profile())
    with patch("agents.a0.research.call_research", return_value=mock_resp):
...
def test_no_judgment_call_for_clean_profile(mock_gemini_clean, jobpilot_root: Path) -> None:
...
def test_judgment_call_written_to_company_profile(jobpilot_root: Path) -> None:
...
def test_no_judgment_call_written_when_clean(mock_gemini_clean, jobpilot_root: Path) -> None:
...
def test_a0_handler_returns_researched_on_success(mock_gemini_clean, jobpilot_root: Path) -> None:
...
def test_a0_handler_returns_research_failed_on_error(
    ...
    from agents.a0.gemini_client import A0ResearchError
    from orchestrator.state_machine import _a0_handler

    with patch(
        "agents.a0.research.call_research",
        side_effect=A0ResearchError("simulated API error"),
    ):
        role = _make_a0_role()
        _write_role_a1(role)
        result = _a0_handler(role)
...
def test_state_transition_logged_to_decisions(mock_gemini_clean, jobpilot_root: Path) -> None:
...
def test_missing_company_domain_raises(jobpilot_root: Path) -> None:
    ...
    role = _make_a0_role()
    role["company_domain"] = ""
    _write_role_a1(make_role("acme-coo-20260424", "researching"))
